    ...
    <section className="rounded-3xl border border-[var(--surface-light)] bg-white p-5">
      ...
      <div className="mt-4 space-y-3">
        {parts.map((part) => (
          <div
            key={part.id}
            className="rounded-2xl border border-[var(--surface-light)] bg-[var(--background)] px-4 py-4 text-sm text-slate-700"
          >
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-slate-500">
              ...
              <button className="rounded-full bg-white px-3 py-1 font-semibold text-[var(--primary-dark)] ">
                Request quote
              </button>
            </div>
          </div>
