...
// Details for the read-only System notifications and Bookings feeds. These are
// generated by the DMS, so there are no members, files or settings — just what
// the feed is, what it currently holds and, for bookings, the request mix.
export function FeedDetails({ feed, onClose }) {
  const { isBookings, notes = [], loading, lastViewedAt, limit } = feed;
  const oldest = notes[0]?.created_at || null;
  const latest = notes[notes.length - 1]?.created_at || null;
  const newCount = lastViewedAt
    ? notes.filter((note) => new Date(note.created_at || 0).getTime() > new Date(lastViewedAt).getTime()).length
    : notes.length;
  ...
  const byType = useMemo(() => {
    ...
    notes.forEach((note) => {
      const label = note.type_label || "Request";
  ...
  return (
    <>
      <div className="app-msg-details__head">
        <h3 className="app-msg-details__title">{isBookings ? "Bookings details" : "Notification details"}</h3>
      ...
      <div className="app-msg-details__scroll custom-scrollbar">
        <Section title="About">
          <dl className="app-msg-facts">
            <dt>Type</dt>
            <dd>{isBookings ? "Customer booking requests" : "DMS system notifications"}</dd>
            <dt>Source</dt>
            <dd>{isBookings ? "Customer website and portal" : "Generated automatically by the DMS"}</dd>
            <dt>Access</dt>
            <dd>Read only</dd>
          </dl>
        </Section>
        ...
        <Section title="Activity">
          {loading ? (
            ...
            <dl className="app-msg-facts">
              <dt>Shown</dt>
              <dd>
                {notes.length} {isBookings ? "request" : "notification"}
                {notes.length === 1 ? "" : "s"}
                {limit ? ` (latest ${limit})` : ""}
              </dd>
              ...
              <dd>{newCount}</dd>
              <dt>Latest</dt>
              <dd>{formatFactDate(latest)}</dd>
              <dt>Oldest shown</dt>
              <dd>{formatFactDate(oldest)}</dd>
              <dt>Last viewed</dt>
              <dd>{lastViewedAt ? formatFactDate(lastViewedAt) : "First visit"}</dd>
            </dl>
          )}
        </Section>
