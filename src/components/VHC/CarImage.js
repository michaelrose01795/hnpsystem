...
const recolorCar = (img, targetRgb, mode) => {
...
export default function CarImage({ style, className, alt = "", ...rest }) {
  const { resolvedMode, accent } = useTheme();
  const mode = resolvedMode === "dark" ? "dark" : "light";
  const [src, setSrc] = useState(CAR_SRC);
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      const recolored = recolorCar(img, targetRgb, mode);
      setSrc(recolored || CAR_SRC);
    };
    img.onerror = () => setSrc(CAR_SRC);
    img.src = CAR_SRC;
  }, [targetRgb, mode]);
