...
const NAV_GROUPS = WORKSPACE_DEPARTMENTS.filter(
  (dept) => dept.category === "general" || dept.category === "departments"
);
...
function SidebarGroupPreview({ groupKey, roles, showRoles = true }) {
  const nav = getDepartmentWorkspaceNav(groupKey, roles);
  const label = nav.label || groupLabel(groupKey);
  const hasContent = nav.dashboards.length > 0 || nav.items.length > 0;

  return (
    <SubSurface style={{ gap: "10px", minWidth: 0 }}>
...
export default function DevSidebarAccess() {
  ...
  const selectedUserGroups = useMemo(
    () => (selectedUser ? getWorkspaceGroups(selectedUserRoles) : []),
  ...
  return (
    <>
      ...
      <Panel
        ...
        actions={<DevButton small onClick={load}>Refresh users</DevButton>}
      >
        <div style={{ fontSize: "var(--text-body-sm)", color: "var(--text-1)", opacity: 0.8 }}>
          Every navigable group, the roles it is assigned to, and its full sidebar — the
          <strong> Dashboards</strong> sub-heading followed by the group-name sub-heading and its page buttons.
        </div>
        <div
          style={{
            display: "grid",
        ...
        >
          {NAV_GROUPS.map((group) => (
            <SidebarGroupPreview key={group.key} groupKey={group.key} roles={ALL_ROLES} />
          ))}
        </div>
      </Panel>

      {/* 2 + 3. USER ACCESS EXPLORER + MULTI-SELECT PREVIEW ----------------- */}
      <Panel title="User access explorer">
        {loading ? (
          <LoadingBlock rows={3} />
        ) : error ? (
          <EmptyState
                  ...
                  <SubSurface style={{ gap: "6px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", marginTop: "8px" }}>
                      <span style={{ fontSize: "var(--text-body-xs)", color: "var(--text-1)", opacity: 0.7 }}>
                        Available groups ({selectedUserGroups.length})
                      </span>
                    ...
                    <div>
                      ...
                      >
                        {selectedUserGroups.map((g) => (
                          <SidebarGroupPreview key={g.key} groupKey={g.key} roles={selectedUserRoles} showRoles={false} />
                        ))}
                      </div>
                    </div>
                  )}
                    ...
                    {previewGroupKeys.length > 0 && (
                      ...
                      >
                        {previewGroupKeys.map((key) => (
                          <SidebarGroupPreview key={key} groupKey={key} roles={ALL_ROLES} />
                        ))}
                      </div>
                    )}
                  </SubSurface>
