// Shared helpers for Presentation mock pages. They keep the demo views close
// to the global app language while staying free of network and auth side effects.
...
export function StatusPill({ status }) {
  const colors = STATUS_COLORS[status] || { bg: "rgba(107,114,128,0.15)", fg: "#374151" };
  return (
    <span
      style={{
...
export function PrimaryBtn({ children, ...rest }) {
  return (
    <button type="button" className="app-btn app-btn--primary" {...rest}>
      {children}
    </button>
  );
}
...
export function GhostBtn({ children, ...rest }) {
  return (
    <button type="button" className="app-btn app-btn--ghost" {...rest}>
      {children}
    </button>
  );
}
