...
export async function updateCustomerProfile(customerId, patch) {
  ...
  // slug_key is a GENERATED column derived from firstname + lastname, so it
  // re-computes itself on a rename and must never be written to — sending it
  // fails the statement with "cannot insert a non-DEFAULT value into column".
  // It is not in `allowed` above, so nothing to strip here.
