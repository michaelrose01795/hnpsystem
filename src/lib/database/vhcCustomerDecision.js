...
import { validateCustomerVhcLink } from "@/lib/database/vhcCustomerReport";
...
export async function updateCustomerVhcDecision(req, res) {
  ...
  try {
    const access = await validateCustomerVhcLink({ jobNumber, linkCode }, dbClient);
    if (access.status !== 200) {
      return res.status(access.status).json({ ...access.body, message: access.body.error });
    }
