...
-- Help & Diagnostics ("support") feature — see src/lib/database/schema/support/000_support.sql
-- for the runnable, idempotent migration (with RLS). Separate from the Reporting/KPI platform.
CREATE TABLE public.support_reports (
  ...
  reporter_roles text[],
  status text NOT NULL DEFAULT 'new'::text CHECK (status IN ('new','triaged','in_progress','resolved','wont_fix','duplicate')),
  severity text NOT NULL DEFAULT 'unset'::text CHECK (severity IN ('unset','low','medium','high','critical')),
  assigned_to integer,
  route text,
  section_key text,
  source_file text,
  ...
  CONSTRAINT support_reports_reporter_fkey FOREIGN KEY (reporter_user_id) REFERENCES public.users(user_id),
  CONSTRAINT support_reports_assigned_fkey FOREIGN KEY (assigned_to) REFERENCES public.users(user_id)
);
CREATE TABLE public.support_report_comments (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  ...
  CONSTRAINT support_report_comments_report_fkey FOREIGN KEY (report_id) REFERENCES public.support_reports(id) ON DELETE CASCADE
);