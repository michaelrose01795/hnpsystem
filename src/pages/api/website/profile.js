...
export default async function handler(req, res) {
  ...
  const [
    ...
    client
      .from("vehicles")
      .select(
        "vehicle_id, reg_number, make, model, year, colour, mileage, fuel_type, transmission, mot_due, warranty_type, warranty_expiry, service_history, insurance_provider, insurance_policy_number",
      )
    ...
    client
      .from("invoices")
      .select(
        "invoice_id, invoice_number, job_number, payment_status, paid, total, grand_total, due_date, created_at",
      )
      .eq("customer_id", customerId)
      .order("created_at", { ascending: false })
      .limit(100),
  ...
  await Promise.all([
    (async () => {
      ...
      const { data } = await client
        .from("vhc_checks")
        .select(
          "job_id, severity, display_status, section, issue_title, issue_description, approval_status",
        )
        .in("job_id", jobIds);
