...
const HolidayList = ({ holidays }) => (
  ...
  >
    {holidays.length === 0 ? (
      <p style={{ margin: 0, color: "#6b7280" }}>No holidays for the coming week.</p>
    ) : (
      holidays.map((absence) => (
        ...
        >
          ...
          <span style={{ color: "#6b7280" }}>{absence.type}</span>
        </div>
      ))
    )}
  </div>
    ...
    <Layout>
      <div style={{ padding: "24px", display: "flex", flexDirection: "column", gap: "20px" }}>
        ...
        <Section title="Upcoming holidays" subtitle="Next 7 days">
          {loading ? (
            <p style={{ color: "#6b7280" }}>Loading holiday requests…</p>
          ) : (
        ...
        <Section title="Notices" subtitle="Latest alerts">
          {loading ? (
            <p style={{ color: "#6b7280" }}>Loading notices…</p>
          ) : (
